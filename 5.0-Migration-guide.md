...
However, all view categories in 5.0 introduce a new extra called `SDWebImageContext`. Which can hold anything that previous enum `SDWebImageOptions` can not, to control advanced option for view loading.

### New Feature

#### Animated Image View

5.0 we introduce a grand new animated image solution. Which including animated image loading, rendering, decoding, and also support customization for advanced user.
...
5.0 introduce the easy way to provide a image transform process, after the image was downloaded from network. Which allow user to easily scale, rotate, rounded corner, or even chain a list of transformers together. These transformed image will also stored to cache to avoid duplicate process. See [Image Transformer](https://github.com/rs/SDWebImage/wiki/Advanced-Usage#transformer-50) for more detailed information.
...
We are no longer hosting the integration with `FLAnimatedImage` inside this repo. We have a dedicated repo for that. See [SDWebImageFLPlugin](https://github.com/SDWebImage/SDWebImageFLPlugin) for more detailed information.
...
#### Swift Specify API Change
SDWebImage 5.0 do a clean up for API. We using many modern Objective-C declaration to generate Swift API. We now provide full nullability support, string enum, class property, and even custom Swift API name. To make the framework works better for our Swift user. Here are the API change specify for Swift. 
